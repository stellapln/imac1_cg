...
void drawCircle(int r, int v, int b, int full) {
    int i;
...
/* Fonction qui dessine l'horloge avec drawSquare et drawCircle et qui prend en paramètre l'heure précise */
void drawClock(int h, int m, int s){
    int i;

    /* Dessin du fond de l'horloge */
    /* BLANC */
    glPushMatrix();
        glColor3ub(10,10,10);
        glScalef(195, 195, 0);
        drawCircle(255,255,255,1);
    glPopMatrix();
    /* NOIR */
    glPushMatrix();
        glScalef(190, 190, 0);
    ...
    /* Heure */
    glPushMatrix();
        glRotatef(-h*30, 0, 0, 1);
        glTranslatef(0,20,0);
        glScalef(4,50,0);
        drawSquare(0,0,0,0,0,1);
    glPopMatrix();
    /* Minutes */
    glPushMatrix();
        glRotatef(-m*6, 0, 0, 1);
    ...
    /* Secondes */
    glPushMatrix();
        glRotatef(-s*6, 0, 0, 1.0);
        glTranslatef(0,35,0);
        glScalef(1,85,0);
        drawSquare(0,0,0,0,0,1);
    glPopMatrix();

    /* Boucle pour l'affichage des traits minutes */
    for(i = 1 ; i <= 60 ; i++){
        if(i%5 == 0){
            glPushMatrix();
                glRotatef(i*6,0,0,1);
...
int main(int argc, char** argv) {
    ...
    /* Titre de la fenêtre */
    SDL_WM_SetCaption("L'horloge du Spoula", NULL);
